...
struct Zipper::Impl {
  ...
  bool initWithStream(std::iostream& stream) {
    ...
    return initMemory(size > 0 ? APPEND_STATUS_CREATE : APPEND_STATUS_ADDINZIP,
                      m_filefunc);
  }
  ...
  bool initWithVector(std::vector<unsigned char>& buffer) {
    ...
    return initMemory(
        buffer.empty() ? APPEND_STATUS_CREATE : APPEND_STATUS_ADDINZIP,
        m_filefunc);
  }
  ...
  bool add(std::istream& input_stream,
    ...
    if (ZIP_OK == err) {
      do {
        ...
        if (size_read < buff.size() && !input_stream.eof() &&
            !input_stream.good())
          err = ZIP_ERRNO;
  ...
  void close() {
    ...
    if (m_zipmem.base && m_zipmem.limit > 0) {
      if (m_outer.m_usingMemoryVector) {
        ...
        m_outer.m_vecbuffer.assign(m_zipmem.base,
                                   m_zipmem.base + m_zipmem.limit);
      }
...
Zipper::Zipper(const std::string& zipname)
    : m_obuffer(*(new std::stringstream()))  // not used but using local
      ...
      ,
      m_vecbuffer(
          *(new std::vector<unsigned char>()))  // not used but using local
                                                // variable throws exception
      ,
